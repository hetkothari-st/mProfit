...
// NOTE (gap, not fixed here): fmvOverride.service.ts imports
// computeUserCapitalGains + GRANDFATHERING_CUTOFF from this file, so this
// import makes the two modules circular. Works under Node ESM live bindings
// since neither side touches the other at module-eval time (only inside
// async function bodies) — but it's a smell worth breaking apart later
// (e.g. moving GRANDFATHERING_CUTOFF/FMV types to a shared module).
