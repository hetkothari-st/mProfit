...
export function buildScanQuery(lookbackFrom: Date, lookbackTo: Date): string {
  // Gmail 'before:' is exclusive (emails ON that date are excluded).
  // Add 1 day so lookbackTo is treated as inclusive by the query.
  const exclusiveTo = new Date(lookbackTo);
  exclusiveTo.setUTCDate(exclusiveTo.getUTCDate() + 1);
  return `${ATTACHMENT_QUERY} after:${formatDate(lookbackFrom)} before:${formatDate(exclusiveTo)}`;
}
