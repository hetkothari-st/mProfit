 ...
 * from a live session against dspim.com, and AMCs move these files without
 ...
 *
 * Everything that can be checked without the network lives in `dsp.parse.ts`
 ...
 * Correct them against the live site, then bump `DSP_ADAPTER_VERSION`.
 */
export const ENDPOINTS = {
  /** Monthly portfolio disclosure, per scheme, per month. UNVERIFIED. */
  portfolioCsv: (schemeCode: string, month: string): string =>
    `https://www.dspim.com/docs/default-source/monthly-portfolio/${month}/${schemeCode}.csv`,
  /** Scheme factsheet page. UNVERIFIED. */
  factsheet: (schemeCode: string): string =>
    `https://www.dspim.com/mutual-fund/${schemeCode}/factsheet`,
} as const;
