 ...
 * from a live session against utimf.com, and AMCs move these files without
 ...
 *
 * Everything that can be checked without the network lives in `uti.parse.ts`
 ...
 * Correct them against the live site, then bump `UTI_ADAPTER_VERSION`.
 */
export const ENDPOINTS = {
  /** Monthly portfolio disclosure, per scheme, per month. UNVERIFIED. */
  portfolioCsv: (schemeCode: string, month: string): string =>
    `https://www.utimf.com/downloads/monthly-portfolio/${month}/${schemeCode}.csv`,
  /** Scheme factsheet page. UNVERIFIED. */
  factsheet: (schemeCode: string): string =>
    `https://www.utimf.com/schemes/${schemeCode}/factsheet`,
} as const;
