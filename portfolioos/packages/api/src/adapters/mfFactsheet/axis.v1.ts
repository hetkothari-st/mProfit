 ...
 * from a live session against axismf.com, and AMCs move these files without
 ...
 *
 * Everything that can be checked without the network lives in `axis.parse.ts`
 ...
 * Correct them against the live site, then bump `AXIS_ADAPTER_VERSION`.
 */
export const ENDPOINTS = {
  /** Monthly portfolio disclosure, per scheme, per month. UNVERIFIED. */
  portfolioCsv: (schemeCode: string, month: string): string =>
    `https://www.axismf.com/statutory-disclosure/monthly-portfolio/${month}/${schemeCode}.csv`,
  /** Scheme factsheet page. UNVERIFIED. */
  factsheet: (schemeCode: string): string =>
    `https://www.axismf.com/mutual-funds/scheme/${schemeCode}/factsheet`,
} as const;
