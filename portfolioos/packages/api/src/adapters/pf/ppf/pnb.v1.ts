...
import { parsePnbPpfPassbook } from './pnb.v1.parse.js';
...
const ID = 'pf.ppf.pnb.v1';
const VERSION = '1.0.0';

const pnbPpfAdapter: PfAdapter = {
  id: ID,
  version: VERSION,
  institution: 'PNB',
  type: 'PPF',
  hostnames: ['netpnb.com'],

  async scrape(ctx: ScrapeContext): Promise<RawScrapePayload> {
    if (process.env.PF_SCRAPE_MOCK === '1') {
      logger.info({ accountId: ctx.account.id }, 'pf.pnb.scrape.mocked');
      return {
        adapterId: ID,
    ...
    if (!ctx.credentials) throw new Error('PNB PPF scrape requires credentials');
    ...
    try {
      ...
      await page.goto('https://www.netpnb.com/', {
      ...
      const tmpPath = join(tmpdir(), `pnb_ppf_${randomUUID()}.pdf`);
...
registerPfAdapter(pnbPpfAdapter);

export { pnbPpfAdapter };
