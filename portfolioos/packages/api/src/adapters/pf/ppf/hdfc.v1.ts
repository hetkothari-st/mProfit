...
import { parseHdfcPpfPassbook } from './hdfc.v1.parse.js';
...
const ID = 'pf.ppf.hdfc.v1';
const VERSION = '1.0.0';

const hdfcPpfAdapter: PfAdapter = {
  id: ID,
  version: VERSION,
  institution: 'HDFC',
  type: 'PPF',
  hostnames: ['netbanking.hdfcbank.com'],

  async scrape(ctx: ScrapeContext): Promise<RawScrapePayload> {
    if (process.env.PF_SCRAPE_MOCK === '1') {
      logger.info({ accountId: ctx.account.id }, 'pf.hdfc.scrape.mocked');
      return {
        adapterId: ID,
    ...
    if (!ctx.credentials) throw new Error('HDFC PPF scrape requires credentials');
    ...
    try {
      ...
      const tmpPath = join(tmpdir(), `hdfc_ppf_${randomUUID()}.pdf`);
  ...
  async parse(raw: RawScrapePayload) {
    ...
    for (const m of raw.members) {
      ...
      const result = parseHdfcPpfPassbook({
        userId: '',
...
registerPfAdapter(hdfcPpfAdapter);

export { hdfcPpfAdapter };
