...
import { parseAxisPpfPassbook } from './axis.v1.parse.js';
...
const ID = 'pf.ppf.axis.v1';
const VERSION = '1.0.0';

const axisPpfAdapter: PfAdapter = {
  id: ID,
  version: VERSION,
  institution: 'AXIS',
  type: 'PPF',
  hostnames: ['omni.axisbank.co.in'],

  async scrape(ctx: ScrapeContext): Promise<RawScrapePayload> {
    if (process.env.PF_SCRAPE_MOCK === '1') {
      logger.info({ accountId: ctx.account.id }, 'pf.axis.scrape.mocked');
      return {
        adapterId: ID,
    ...
    if (!ctx.credentials) throw new Error('Axis PPF scrape requires credentials');
    ...
    try {
      ...
      const tmpPath = join(tmpdir(), `axis_ppf_${randomUUID()}.pdf`);
  ...
  async parse(raw: RawScrapePayload) {
    ...
    for (const m of raw.members) {
      ...
      const result = parseAxisPpfPassbook({
        userId: '',
...
registerPfAdapter(axisPpfAdapter);

export { axisPpfAdapter };
