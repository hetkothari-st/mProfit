...
import { parseBobPpfPassbook } from './bob.v1.parse.js';
...
const ID = 'pf.ppf.bob.v1';
const VERSION = '1.0.0';

const bobPpfAdapter: PfAdapter = {
  id: ID,
  version: VERSION,
  institution: 'BOB',
  type: 'PPF',
  hostnames: ['bobnetbanking.bankofbaroda.in'],

  async scrape(ctx: ScrapeContext): Promise<RawScrapePayload> {
    if (process.env.PF_SCRAPE_MOCK === '1') {
      logger.info({ accountId: ctx.account.id }, 'pf.bob.scrape.mocked');
      return {
        adapterId: ID,
    ...
    if (!ctx.credentials) throw new Error('Bank of Baroda PPF scrape requires credentials');
    ...
    try {
      ...
      await page.goto('https://www.bobnetbanking.bankofbaroda.in/', {
      ...
      const tmpPath = join(tmpdir(), `bob_ppf_${randomUUID()}.pdf`);
  ...
  async parse(raw: RawScrapePayload) {
    ...
    for (const m of raw.members) {
      ...
      const result = parseBobPpfPassbook({
        userId: '',
...
registerPfAdapter(bobPpfAdapter);

export { bobPpfAdapter };
