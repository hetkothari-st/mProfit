...
import { parseIciciPpfPassbook } from './icici.v1.parse.js';
...
const ID = 'pf.ppf.icici.v1';
const VERSION = '1.0.0';

const iciciPpfAdapter: PfAdapter = {
  id: ID,
  version: VERSION,
  institution: 'ICICI',
  type: 'PPF',
  hostnames: ['infinity.icicibank.com'],

  async scrape(ctx: ScrapeContext): Promise<RawScrapePayload> {
    if (process.env.PF_SCRAPE_MOCK === '1') {
      logger.info({ accountId: ctx.account.id }, 'pf.icici.scrape.mocked');
      return {
        adapterId: ID,
    ...
    if (!ctx.credentials) throw new Error('ICICI PPF scrape requires credentials');
    ...
    try {
      ...
      await page.goto('https://infinity.icicibank.com/corp/AuthenticationController', {
      ...
      const tmpPath = join(tmpdir(), `icici_ppf_${randomUUID()}.pdf`);
...
registerPfAdapter(iciciPpfAdapter);

export { iciciPpfAdapter };
